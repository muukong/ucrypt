...
int s_uc_div_school_old(uc_int *c, uc_int *d, uc_int *a, uc_int *b)
{
    ...
    if ((err = uc_init(&t1)) != UC_OK)                           goto LBL_Q;
    if ((err = uc_init(&t2)) != UC_OK)                           goto LBL_T1;
    uc_init(&x);
    uc_grow(&t1, 1600);
    uc_grow(&t2, 1600); // Q: how big should this be?
    if ((err = uc_copy(&x, a)) != UC_OK)                    goto LBL_T2;
    uc_init(&y);
    if ((err = uc_copy(&y, b)) != UC_OK)                    goto LBL_X;

    /* fix the sign */
    neg = (a->sign != b->sign);
    x.sign = y.sign = UC_POS;

    /* normalize both x and y, ensure that y >= b/2, [b == 2**MP_DIGIT_BIT] */
    norm = uc_count_bits(&y) % UC_DIGIT_BITS;
    if (norm < (UC_DIGIT_BITS - 1)) {
        norm = (UC_DIGIT_BITS - 1) - norm;
        //if ((err = uc_mul_2d(&x, norm, &x)) != UC_OK)             goto LBL_Y;
        //if ((err = uc_mul_2d(&y, norm, &y)) != UC_OK)             goto LBL_Y;
        if ((err = uc_lshb(&x, &x, norm)) != UC_OK)             goto LBL_Y;
        if ((err = uc_lshb(&y, &y, norm)) != UC_OK)             goto LBL_Y;
    } else {
        norm = 0;
    }
    printf("norm = %d\n", norm);
    ...
    /* y = y*b**{n-t} */
    if ((err = uc_lshd(&y, &y, n - t)) != UC_OK)                     goto LBL_Y;

    while (uc_cmp(&x, &y) != UC_LT) {
        ++(q.digits[n - t]);
        //if ((err = uc_sub(&x, &y, &x)) != UC_OK)                  goto LBL_Y;
        if ((err = uc_sub(&x, &x, &y)) != UC_OK)                  goto LBL_Y;
    }
    ...
    for (i = n; i >= (t + 1); i--) {
        ...
        if (x.digits[i] == y.digits[t]) {
            q.digits[(i - t) - 1] = ((uc_digit)1 << (uc_digit)UC_DIGIT_BITS) - (uc_digit)1;
        } else {
            uc_word tmp;
            tmp = (uc_word)x.digits[i] << (uc_word)UC_DIGIT_BITS;
            tmp |= (uc_word)x.digits[i - 1];
            tmp /= (uc_word)y.digits[t];
            if (tmp > (uc_word)UC_DIGIT_MASK) {
                tmp = UC_DIGIT_MASK;
            }
        ...
        /* while (q{i-t-1} * (yt * b + y{t-1})) >
           ...
           do q{i-t-1} -= 1;
        */
        q.digits[(i - t) - 1] = (q.digits[(i - t) - 1] + 1uL) & (uc_digit)UC_DIGIT_MASK;
        do {
            q.digits[(i - t) - 1] = (q.digits[(i - t) - 1] - 1uL) & (uc_digit)UC_DIGIT_MASK;

            /* find left hand */
            uc_set_zero(&t1);
            t1.digits[0] = ((t - 1) < 0) ? 0u : y.digits[t - 1];
            t1.digits[1] = y.digits[t];
            t1.used = 2;
            //if ((err = uc_mul_d(&t1, q.digits[(i - t) - 1], &t1)) != UC_OK)   goto LBL_Y;
            if ((err = uc_mul_d(&t1, &t1, q.digits[(i - t) - 1])) != UC_OK)   goto LBL_Y;

            /* find right hand */
            t2.digits[0] = ((i - 2) < 0) ? 0u : x.digits[i - 2];
            t2.digits[1] = x.digits[i - 1]; /* i >= 1 always holds */
            t2.digits[2] = x.digits[i];
            t2.used = 3;
            //printf("t1 = "); uc_debug_print_int_radix(&t1, 10);
            //printf("t2 = "); uc_debug_print_int_radix(&t2, 10);
        } while (uc_cmp_mag(&t1, &t2) == UC_GT); /* while |t1| >= |t2| */
        ...
        if (uc_is_neg(&x)) {
            if ((err = uc_copy(&y, &t1)) != UC_OK)                        goto LBL_Y;
            if ((err = uc_lshd(&t1, &t1, (i - t) - 1)) != UC_OK)               goto LBL_Y;
            if ((err = uc_add(&x, &x, &t1)) != UC_OK)                     goto LBL_Y;
    ...
    x.sign = uc_is_zero(&x) ? UC_POS : a->sign;

    if (c != NULL) {
        uc_clamp(&q);
        uc_exch(&q, c);
        c->sign = (neg ? UC_NEG : UC_POS);
    }

    if (d != NULL) {
        //if ((err = uc_div_2d(&x, norm, &x, NULL)) != UC_OK)       goto LBL_Y;
        if ((err = uc_rshb(&x, &x, norm)) != UC_OK)       goto LBL_Y;
        uc_exch(&x, d);
    }
